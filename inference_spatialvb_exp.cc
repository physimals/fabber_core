...
void SpatialVariationalBayesExp::GetOptions(vector<OptionSpec> &opts) const
{
...
InferenceTechnique *SpatialVariationalBayesExp::NewInstance()
{
    return new SpatialVariationalBayesExp();
}
...
void SpatialVariationalBayesExp::Initialize(FwdModel *fwd_model,
    FabberRunData &args)
{
    ...
    if (!m_use_full_evidence && !args.GetBool("no-eo") && m_prior_types_str.find_first_of("DR") != string::npos)
    {
        ...
        if (!m_use_sim_evidence)
            WARN_ONCE("SpatialVariationalBayesExp::Initialize Defaulting to Full (non-simultaneous) Evidence Optimization");
    }
...
void SpatialVariationalBayesExp::SetupPerVoxelDists(FabberRunData &allData)
{
...
void SpatialVariationalBayesExp::SetupStSMatrix()
{
    assert((int)m_neighbours.size() == m_nvoxels);

    const double tiny = 1e-6;
    WARN_ONCE(
        "SpatialVariationalBayesExp::SetupStSMatrix Using 'S' prior with fast-calculation method and constant "
        "diagonal weight of "
        + stringify(tiny));
    // NEW METHOD
    {
        LOG << "SpatialVariationalBayesExp::SetupStSMatrix Attempting to allocate, m_nvoxels = " << m_nvoxels << endl;
        ...
        LOG << "SpatialVariationalBayesExp::Done generating StS matrix (New method)" << endl;
    }
}

void SpatialVariationalBayesExp::UpdateAkmean(DiagonalMatrix &akmean)
{
    LOG << "SpatialVariationalBayesExp::UpdateAkmean" << endl;
    ...
    for (int k = 1; k <= m_num_params; k++)
    {
        ...
        case 'Z':
        {
            ...
            WARN_ONCE("SpatialVariationalBayesExp::UpdateAkmean Hyperpriors on S prior: using q1 == " + stringify(q1) + ", q2 == " + stringify(q2));
...

        {
            ...
            LOG << "SpatialVariationalBayesExp::UpdateAkmean k=" << k << ", tmp1=" << tmp1 << ", tmp2=" << tmp2 << endl;
            // LOG << Swk.t();
    ...
    for (int k = 1; k <= akmean.Nrows(); k++)
    {
        if (akmean(k, k) < 1e-50)
        {
            LOG << "SpatialVariationalBayesExp::UpdateAkmean akmean(" << k << ") was " << akmean(k, k) << endl;
            WARN_ONCE("SpatialVariationalBayesExp::UpdateAkmean akmean value was tiny!");
        ...
        {
            LOG << "SpatialVariationalBayesExp::UpdateAkmean Rate-limiting the increase on akmean " << k << ": was "
                << akmean(k) << ", now " << akmeanMax(k) << endl;
            akmean(k) = akmeanMax(k);
        }
    }

    LOG << "SpatialVariationalBayesExp::UpdateAkmean New akmean: " << akmean.AsColumn().t();
}

void SpatialVariationalBayesExp::UpdateDeltaRho(DiagonalMatrix &delta, DiagonalMatrix &rho, const DiagonalMatrix &akmean, bool first_iter)
{
    for (int k = 1; k <= m_num_params; k++)
    {
        // if(k<6){LOG_ERR("Skipping parameter "<<k<<endl);continue;}
        LOG << "SpatialVariationalBayesExp::UpdateDeltaRho Optimizing for parameter " << k << endl;
        ...
        case 'A':
            ...
            LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type " << type << endl;
            break;
    ...
    == m_fwd_prior.at(v-1).GetPrecisions()(k,k));
            ...
            if (first_iter && !m_update_first_iter)
            {
                if (type == 'F' && m_brute_force_delta_search)
                    LOG << "SpatialVariationalBayesExp::UpdateDeltaRho Doing calc on first iteration, just because it's F and "
            ...
            if (type == 'R')
            {
                ...
                if (m_use_evidence)
                {
                    WARN_ALWAYS("SpatialVariationalBayesExp::UpdateDeltaRho Using R... mistake??");
                    delta(k) = OptimizeEvidence(m_fwd_post_no_prior, k,
                        initialFwdPrior.get(), delta(k), true,
                        &rho(k));
                    LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type R eo : " << delta(k)
                        << " " << rho(k) << " 0" << endl;
                }
                else
                {
                    WARN_ALWAYS("SpatialVariationalBayesExp::UpdateDeltaRho Using R without EO... mistake??");
                    // Spatial priors with rho & delta
                    delta(k) = OptimizeSmoothingScale(covRatio, meanDiffRatio,
                        delta(k), &rho(k), true);
                    LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type R vb : " << delta(k)
                        << " " << rho(k) << " 0" << endl;
                }
            }
            else if (type == 'D')
            {
                ...
                if (m_use_evidence)
                {
                    ...
                    LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type D eo : " << delta(k)
                        << " 0 0" << endl;
                }
                else
                {
                    WARN_ALWAYS("SpatialVariationalBayesExp::UpdateDeltaRho Using D without EO... mistake??");
                    delta(k) = OptimizeSmoothingScale(covRatio, meanDiffRatio,
                        delta(k), &rho(k), false);
                    LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type D vb : " << delta(k)
                        << " 0 0" << endl;
                }
            }
            else // type == 'F'
            {
                ...
                deltaMax(k) = delta(k);
                LOG << "SpatialVariationalBayesExp::SpatialPrior " << k << " type F : " << delta(k) << " "
                    << rho(k) << " 0" << endl;
            }
            ...
            {
                LOG << "SpatialVariationalBayesExp::Rate-limiting the increase on delta " << k << ": was "
                    << delta(k);
                delta(k) = deltaMax(k);
        ...
        LOG << "SpatialVariationalBayesExp::UpdateDeltaRho delta(k) = " << delta(k) << ", rho(k) == " << rho(k)
            << endl;
    }
}

// CALCULATE THE C^-1 FOR THE NEW DELTAS
void SpatialVariationalBayesExp::CalculateCinv(vector<SymmetricMatrix> &Sinvs, DiagonalMatrix &delta, DiagonalMatrix &rho, DiagonalMatrix &akmean)
{
    LOG << "SpatialVariationalBayesExp::CalculateCinv" << endl;
    for (int k = 1; k <= m_num_params; k++)
    {
        if (delta(k) >= 0)
        {
...
void SpatialVariationalBayesExp::DoSimEvidence(vector<SymmetricMatrix> &Sinvs)
{
    LOG << "SpatialVariationalBayesExp::DoSimEvidence";
    ...
    if (!(initialFwdPrior->means == -initialFwdPrior->means))
        WARN_ALWAYS("SpatialVariationalBayesExp::DoSimEvidence Quick hack to avoid assertion with initialFwdPrior->means != 0");
    ...
    for (int v = 1; v <= m_nvoxels; v++)
    {
        ...
        if (m_use_covar_marginals_not_precisions)
        {
            ...
            WARN_ONCE("SpatialVariationalBayesExp::DoSimEvidence Full simultaneous diagonal thingy -- now in covariances!");
            ...
            if ((cov - covOld).MaximumAbsoluteValue() > 1e-10)
                LOG << "SpatialVariationalBayesExp::DoSimEvidence covBefore: " << covOld.AsColumn().t()
        ...
        else
        {
            ...
            WARN_ONCE("SpatialVariationalBayesExp::DoSimEvidence Full simultaneous diagonal thingy");
            // LOG << "prec = \n" << prec << endl;
            for (int k1 = 1; k1 <= m_num_params; k1++)
            {
                for (int k2 = 1; k2 <= m_num_params; k2++)
            ...
            if ((prec - precOld).MaximumAbsoluteValue() > 1e-10)
                LOG << "SpatialVariationalBayesExp::DoSimEvidence precBefore: " << precOld.AsColumn().t()
...
void SpatialVariationalBayesExp::DoFullEvidence(vector<SymmetricMatrix> &Sinvs)
{
    //	assert(!m_use_covar_marginals_not_precisions);
    // Covariance marginals are broken below, and I think they're
    // rubbish anyway
    LOG << "SpatialVariationalBayesExp::DoFullEvidence using " + string(m_use_covar_marginals_not_precisions
                                                                      ? "covariances."
                                                                      : "precisions.");
    ...
    for (int k = 1; k <= m_num_params; k++)
    {
        ...
        for (int v = 1; v <= m_nvoxels; v++)
        {
            ...
            WARN_ONCE(
                "SpatialVariationalBayesExp::DoFullEvidence Corrected mistake in useFullEvidenceOptimization: "
    ...
    for (int v = 1; v <= m_nvoxels; v++)
    {
        ...
        if (m_use_covar_marginals_not_precisions)
        {
            ...
            WARN_ONCE("SpatialVariationalBayesExp::DoFullEvidence Covariance diagonal thingy");
            // LOG << "cov = \n" << cov << endl;
        ...
        {
            WARN_ONCE("SpatialVariationalBayesExp::DoFullEvidence Keeping inter-parameter covariances from VB!");
        }
        else
        {
            ...
            WARN_ONCE("SpatialVariationalBayesExp::DoFullEvidence Precision diagonal thingy");
            // LOG << "prec = \n" << prec << endl;
            for (int k = m_full_eo_first_param; k <= m_num_params; k++)
                prec(k, k) = SigmaInv[k - 1](v, v);

            if ((prec - precOld).MaximumAbsoluteValue() > 1e-10)
                LOG << "SpatialVariationalBayesExp::DoFullEvidence precBefore: " << precOld.AsColumn().t()
                    << "precAfter: " << prec.AsColumn().t();
...
void SpatialVariationalBayesExp::SetFwdPriorShrinkageTypeS(int v, const NEWMAT::DiagonalMatrix &akmean)
{
    WARN_ONCE("SpatialVariationalBayesExp::SetFwdPriorShrinkageTypeS Using new S VB spatial thingy");
...
void SpatialVariationalBayesExp::SetFwdPriorShrinkageType(int v, const NEWMAT::DiagonalMatrix &akmean)
{
    const double tiny = 0; // turns out to be no longer necessary.
    ...
    else if (m_shrinkage_type == 'S')
    {
        ...
        WARN_ONCE("SpatialVariationalBayesExp::SetFwdPriorShrinkageType Using a hacked-together VB version of the 'S' prior");
    }
...
double SpatialVariationalBayesExp::SetFwdPrior(int v, const vector<SymmetricMatrix> &Sinvs, bool isFirstIteration)
{
...
void SpatialVariationalBayesExp::DoCalculations(FabberRunData &allData)
{
    ...
    rho = 0;
    LOG << "SpatialVariationalBayesExp::Using initial value for all deltas: " << delta(1) << endl;
    ...
    do
    {
        ...
        for (int v = 1; v <= m_nvoxels; v++)
        {
            ...
            LOG << "1. Prior means: " << m_fwd_prior[v-1].means.t() << endl;
            LOG << "1. Posterior means: " << m_fwd_post[v-1].means.t() << endl;
            ...
            LOG << "2. Prior means: " << m_fwd_prior[v-1].means.t() << endl;
            LOG << "2. Posterior means: " << m_fwd_post[v-1].means.t() << endl;
            ...
            LOG << "4. Prior means: " << m_fwd_prior[v-1].means.t() << endl;
            LOG << "4. Posterior means: " << m_fwd_post[v-1].means.t() << endl;
        ...
        for (int v = 1; v <= m_nvoxels; v++)
        {
            ...
            LOG << "5. Prior means: " << m_fwd_prior[v-1].means.t() << endl;
    ...
    for (int k = 1; k <= m_num_params; k++)
    {
        ...
        LOG << "SpatialVariationalBayesExp::Coefficient resels per voxel for param "
    ...
    if (m_save_spatial_priors)
    {
        ...
        LOG << "SpatialVariationalBayesExp::Not saving spatial priors as not implemented" << endl;
#if 0
		Matrix vols;
...
void SpatialVariationalBayesExp::CheckCoordMatrixCorrectlyOrdered(const Matrix &coords)
{
    // Only 3D
 ...
 */
void SpatialVariationalBayesExp::CalcNeighbours(const Matrix &coords)
{
...
double SpatialVariationalBayesExp::OptimizeEvidence(
    ...
    double *rhoOut) const
{
    ...
    WARN_ONCE("SpatialVariationalBayesExp::OptimizeEvidence Hard limits on delta: [" + stringify(hardMin) + ", " + stringify(hardMax) + "]");
...
double SpatialVariationalBayesExp::OptimizeSmoothingScale(
    const DiagonalMatrix &covRatio,
    const ColumnVector &meanDiffRatio,
    double guess,
    double *optimizedRho,
    ...
    bool allowDeltaToVary) const
{
    ...
    {
        LOG << "SpatialVariationalBayesExp::BEGINNING BRUTE-FORCE DELTA SEARCH" << endl;
        LOG << "SpatialVariationalBayesExp::PARAMETERS:\ncovRatio = [" << covRatio << endl
            << "];\nmeanDiffRatio = [" << meanDiffRatio << "];\n";

        // BELOW: changed cutoff (was 1e16 originally)
        for (double dk = 0.001; dk < 1e4; dk *= (sqrt((2))))
        {
            LOG << "SpatialVariationalBayesExp::dk = " << dk << endl;
            LOG << "SpatialVariationalBayesExp::BRUTEFORCE=" << dk << "\t"
        ...
        LOG << "SpatialVariationalBayesExp::END OF BRUTE-FORCE DELTA SEARCH" << endl;
    }
