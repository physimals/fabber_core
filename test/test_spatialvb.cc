        ...
        for (int y = 0; y < VSIZE; y++)
        {
            for (int x = 0; x < VSIZE; x++)
            {
                ...
                if (x != VSIZE - 1)
                    expected.push_back(v + 1);
                if (y != 0)
                    expected.push_back(v - VSIZE);
                if (y != VSIZE - 1)
                    expected.push_back(v + VSIZE);
                if (z != 0)
                    expected.push_back(v - VSIZE * VSIZE);
                if (z != VSIZE - 1)
                    expected.push_back(v + VSIZE * VSIZE);
        ...
        for (int y = 0; y < VSIZE; y++)
        {
            for (int x = 0; x < VSIZE; x++)
            {
                ...
                if (x <= VSIZE - 3)
                    expected.push_back(v + 2);
                if (y >= 2)
                    expected.push_back(v - (2 * VSIZE));
                if (y <= VSIZE - 3)
                    expected.push_back(v + (2 * VSIZE));
                if (z >= 2)
                    expected.push_back(v - (2 * VSIZE * VSIZE));
                if (z <= VSIZE - 3)
                    expected.push_back(v + (2 * VSIZE * VSIZE));
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if ((x <= VSIZE - 2) && (y >= 1))
                        expected.push_back(v + 1 - VSIZE);
                    if ((x <= VSIZE - 2) && (y <= VSIZE - 2))
                        expected.push_back(v + 1 + VSIZE);

                    if ((x >= 1) && (z >= 1))
                        expected.push_back(v - 1 - (VSIZE * VSIZE));
                    if ((x >= 1) && (z <= VSIZE - 2))
                        expected.push_back(v - 1 + (VSIZE * VSIZE));
                    if ((x <= VSIZE - 2) && (z >= 1))
                        expected.push_back(v + 1 - (VSIZE * VSIZE));
                    if ((x <= VSIZE - 2) && (z <= VSIZE - 2))
                        expected.push_back(v + 1 + (VSIZE * VSIZE));

                    if ((y >= 1) && (z >= 1))
                        expected.push_back(v - VSIZE - (VSIZE * VSIZE));
                    if ((y >= 1) && (z <= VSIZE - 2))
                        expected.push_back(v - VSIZE + (VSIZE * VSIZE));
                    if ((y <= VSIZE - 2) && (z >= 1))
                        expected.push_back(v + VSIZE - (VSIZE * VSIZE));
                    if ((y <= VSIZE - 2) && (z <= VSIZE - 2))
                        expected.push_back(v + VSIZE + (VSIZE * VSIZE));
                }
                //                cout << "voxel " << v << " expected=" << expected.size() << endl;
