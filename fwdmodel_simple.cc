...
void SimpleFwdModel::Evaluate(const ColumnVector& params, ColumnVector& result) const
{
	...
	ColumnVector tcModulated = id.Q0(params) * (1 + basis * id.Qn(params) / 100.0);
	ColumnVector tcUnmodulated = id.U0(params) * (1 + basis * id.Un(params) / 100.0);
...
void SimpleFwdModel::DumpParameters(const ColumnVector& vec, const string& indent) const
{
