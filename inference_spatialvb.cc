...
void SpatialVariationalBayes::DoCalculations(FabberRunData& allData)
{
...

    {
        ...
        if (lockedLinearEnabled) {
            ...
            for (int v = 1; v <= Nvoxels; v++) {
                lockedLinearCentres.Column(v) = lockedLinearDists.at(v - 1)->means.Rows(1, Nparams);
            }
        }
    ...
    do {
        ...
        for (int v = 1; v <= Nvoxels; v++) {
            ...
            if (shrinkageType == 'S') {
                ...
                ColumnVector contrib12(Nparams);
                contrib12 = 0.0;
                for (vector<int>::iterator nidIt = neighbours2[v - 1].begin(); nidIt != neighbours2[v - 1].end(); nidIt++)
                // iterate over neighbour ids
                {
            ...
            if (needF) {
                F = noise->CalcFreeEnergy(*noiseVox[v - 1], *noiseVoxPrior[v - 1], fwdPosteriorVox[v - 1],
                        fwdPriorVox[v - 1], linearVox[v - 1], data.Column(v));
                F += Fard;
            }
        ...
        if (useSimultaneousEvidenceOptimization) {
            ...
            ColumnVector XYtr(Nvoxels * Nparams);
            XYtr = -999;
            ...
            for (int v = 1; v <= Nvoxels; v++) {
                ...
                if (useCovarianceMarginalsRatherThanPrecisions) {
                    SymmetricMatrix cov = SP(fwdPosteriorVox[v - 1].GetCovariance(), IdentityMatrix(Nparams));
                    ...
                    SymmetricMatrix prec = SP(fwdPosteriorVox[v - 1].GetPrecisions(), IdentityMatrix(Nparams));
    ...
    if (alsoSaveSpatialPriors) {
        ...
        //        data.SaveVoxelData("finalSpatialPriors", vols, NIFTI_INTENT_SYMMATRIX);
#endif
    }
 ...
 */
void SpatialVariationalBayes::CalcNeighbours2(const Matrix& voxelCoords)
{
    ...
    int xsize = (int)voxelCoords.Row(2).Maximum();
    int ysize = (int)voxelCoords.Row(2).Maximum();
    int zsize = (int)voxelCoords.Row(3).Maximum();
    vector<int> mask;

    for (int v = 1; v <= nVoxels; v++) {
        offset = voxelCoords(1, v) + voxelCoords(2, v)*ysize + voxelCoords(3, v)*ysize*zsize;
        assert(maskVox == 0);
        maskVox = v;
    }
...
/**
 * Calculate nearest and second-nearest neighbours for the voxels specified by the mask
 * 
 * @param mask identifies the voxels of interest.
 */
void SpatialVariationalBayes::CalcNeighbours(const NEWIMAGE::volume<float>& mask)
{
    Tracer_Plus tr("SpatialVariationalBayes::CalcNeighbours");

    // Create a column vector with one entry per voxel. The number of
    // voxels is the number of points in the mask with a value of 1 rather than 0
    ColumnVector preThresh((int) mask.sum());
    const int nVoxels = preThresh.Nrows();

    // Probably because mask is empty, logic error
    assert(nVoxels > 0);

    // Populate preThresh with the offset into the
    ...
    // Go through each voxel. Note that preThresh indexed from 1 not 0.
    for (int vid = 1; vid <= nVoxels; vid++) {

        // Get the voxel offset into the matrix
        int pos = int(preThresh(vid));
//        cout << "voxel " << vid << " offset= " << pos << endl;
        // Now search for neighbours
        for (unsigned n = 0; n < delta.size(); n++) {

            // ids there a voxel at this neighbour position?
            // indexed from 1; id == -1 if not found.
            int id = binarySearch(preThresh, pos + delta[n]);

//            cout << "checking voxel " << id << " at offset " << delta[n] << endl;
            ...
            // FIXME are these offsets starting at 0 or 1? Looks wrong if starting at 0
            //
            // On investigation, yes the algorithm is wrong, however it works provided it is called
            // on data which does not have x,y,z coordinates of zero. Effectively the mask prevents
            // wrap around in this case because there are no voxels in the wrap around positions.
            //
            // Still I think this needs fixing
#if 0
            if ((delta.size() > n + 2) && // Don't check for wrap around on last co-ord as not possible
                    (((delta[n] > 0) && (pos % delta[n + 2] == 0)) || // For positive offset, check not at end of row
                            ((delta[n] < 0) && (pos % -delta[n + 2] == 1)))) { // For negative offset, check not at start of row
#endif
            if (delta.size() >= n + 2) {
                ...
                if (delta[n] > 0) {
                    int test = delta[n+2];
                    if (test > 0) ignore = (pos % test) >= test - delta[n];
                }
                else {
                    int test = -delta[n+2];
                    if (test > 0) ignore = (pos % test) < -delta[n];
                }
                if (ignore) {
//                    cout << "ignoring" << endl;
                    continue;
...
//            cout << "voxel " << vid << "has neighbour" << id << endl;
    ...
    // (diagonally connected) FIXME intentional?
    neighbours2.resize(nVoxels);
