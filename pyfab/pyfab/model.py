...
from matplotlib.compat.subprocess import STDOUT
...
class Run:
    ...
    def __init__(self, dir, shape=[]):
        ...
        try:
            self.time = datetime.datetime.strptime(self.timestamp, "%c")
        except:
            print("Failed to parse timestamp")
            self.time = os.path.getmtime(self.logfile)
...
class RunFile:
    ...
    def get_runs(self):
        ...
        while True:
            ...
            try:
                ...
                print("Could not find logfile in: %s - ignoring" % dir)
            dir += "+"
        self.runs.sort(key=lambda run: run.time)
        return self.runs
    ...
    def select_run(self, dir=None):
        ...
        # Select the data with the same key in the new run
        if self.current_data: self.select(self.current_data.role)
        self._change("selection", "data")
        self._update_views()
    ...
    def _set_data(self, key, value):
        """
        Set an option which is the name of a data file
        
        It will make the path absolute if it isn't already.
    ...
    def _run_full(self, quick=False):
        """
        ...
        print(self.fabdir)
        err = ""
        p = sub.Popen(cmd, stdout=sub.PIPE, stderr=sub.PIPE, cwd=self.fabdir)
        while 1:
            (stdout, stderr) = p.communicate()
            status = p.poll()
            print stdout 
