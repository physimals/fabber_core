...
class OutputDataView(ModelView):
	def __init__(self, dialog, **kwargs):
		ModelView.__init__(self, **kwargs)
		self.logViewer = dialog
		self.table.setHorizontalHeaderItem(0, QTableWidgetItem("Output Data"))	
		self.runCombo.currentIndexChanged.connect(self.run_changed)
		self.logBtn.clicked.connect(self.view_log_pressed)
		self.table.currentItemChanged.connect(self.data_select)
		self.cur_timestamp = ""

	def run_changed(self, cur):
		for run in self.fab.runs:
			if run.timestamp == self.runCombo.itemText(cur):
				self.fab.select_run(run.dir)

	def view_log_pressed(self):
		self.logViewer.show()
		self.logViewer._raise()
	...
	def relist(self):
		...
		if self.fab.current_run:
			...
			for key, data in data.items():
				...
				self.table.setItem(n, 0, item)
				n += 1
			self.logViewer.textBrowser.setText(self.fab.current_run.get_log())
	...
	def do_update(self):
		self.runCombo.clear()
		n, sel, sel_timestamp = 0, 0, ""
		
		for run in self.fab.runs:
			self.runCombo.insertItem(n, run.timestamp)
			if self.fab.current_run and run.dir == self.fab.current_run.dir:
				sel = n
				sel_timestamp = self.fab.current_run.timestamp
			n += 1
		self.runCombo.setCurrentIndex(sel)

		if sel_timestamp != self.cur_timestamp:
			self.cur_timestamp = sel_timestamp
			self.relist()
...
class FileView(ModelView):
	...
	def run(self):
		self.fab.run(quick=False)
		
	def run_quick(self):
		self.fab.run(quick=True)
