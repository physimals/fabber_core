...
public:
	 ...
	 */
	void SaveVoxelData(std::string filename, NEWMAT::Matrix &coords,
			int nifti_intent_code = NIFTI_INTENT_NONE);
...
#ifdef USE_NEWIMAGE
	/**
	 * Load NIFTI data using the NEWIMAGE library
	 *
	 * @param args Parameters describing the data filenames, etc
	 */
...
private:
	...
	void LoadVoxelCoords(std::string mask_filename);
	void LoadVoxelDataMultiple(std::vector<std::string> &filenames,
...
template<typename T> inline T convertTo(const std::string& s)
{
	...
	char c;
	if (!(i >> x) || (i.get(c)))
		throw Invalid_option("convertTo failed... couldn't convert string '"
				+ s + "' into a value.\n");
	return x;
}
